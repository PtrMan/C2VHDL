...
class Parser:
  ...
  def parse_input(self, name):
    input_name = name.lstrip("input_")
    self.tokens.expect("(")
    self.tokens.expect(")")
    return Input(input_name)

  def parse_output(self, name):
    output_name = name.lstrip("output_")
    self.tokens.expect("(")
    expression = self.parse_expression()
    self.tokens.expect(")")
...
#Expressions may also provide a value method which returns the value of an expression
...
#Since we are generating code for a hardware implementation, we do not need to limit oursleves to
...
#instructions. In the final implementation, all instructions within a frame are executed concurently.

#The function looks at the dependencies between one instruction and another to determine whether
...
def generate_VHDL(name, frames, output_file, registers, arrays):
  ...
  signals.extend(["  signal OUTPUT_S_%s_STB : std_logic;\n"%i.upper() for i in outputs])
  signals.extend(["  signal INPUT_S_%s_ACK : std_logic;\n"%i.upper() for i in inputs])
  ...
  for location, frame in enumerate(frames):
    ...
    for instruction in frame:
      ...
      if instruction["op"] == "literal":
        ...
        output_file.write("        PROGRAM_COUNTER <= %s;\n"%location)
        output_file.write("        INPUT_S_%s_ACK <= '1';\n"%instruction["input"])
        output_file.write( "        if INPUT_S_%s_ACK = '1' and INPUT_%s_STB = '1' then\n"%(
          instruction["input"],
          instruction["input"]))
        output_file.write("          INPUT_S_%s_ACK <= '0';\n"%instruction["input"])
        output_file.write("          PROGRAM_COUNTER <= %s;\n"%(location+1))
        output_file.write("        end if;\n")
        ...
        output_file.write("        PROGRAM_COUNTER <= %s;\n"%location)
        output_file.write("        OUTPUT_S_%s_STB <= '1';\n"%instruction["output"])
        output_file.write(
          "        if OUTPUT_S_%s_STB = '1' and OUTPUT_%s_ACK = '1' then\n"%(
          instruction["output"],
          instruction["output"]))
        output_file.write("          OUTPUT_S_%s_STB <= '0';\n"%instruction["output"])
        output_file.write("          PROGRAM_COUNTER <= %s;\n"%(location+1))
        output_file.write("        end if;\n")
  ...
  buffers = []
  buffers.extend(["  OUTPUT_%s_STB <= OUTPUT_S_%s_STB;\n"%(i.upper(),i.upper()) for i in outputs])
  buffers.extend(["  INPUT_%s_ACK <= INPUT_S_%s_ACK;\n"%(i.upper(),i.upper()) for i in inputs])
...
if __name__ == "__main__":
  ...
  if len(sys.argv) < 2:
    print "Usage: cc.py [options] <input_file>"
    print
    ...
    print "  run           : runs compiled code, used with ghdl or modelsim options"
    sys.exit(-1)
  ...
  generate_VHDL(name, frames, output_file, parser.allocator.all_registers, parser.allocator.all_arrays)
  output_file.close()

  #optionaly, run the design using the simulator of your choice.
  if "ghdl" in sys.argv:
    import os
    os.system("ghdl -a %s.vhd"%name)
